...
# ZeroGPU (the free hardware tier for this Space) requires at least one
# @spaces.GPU-decorated function that Gradio actually binds to a handler --
# the startup scan walks registered .click()/.submit() callbacks, not just
# any decorated function in the module. This app has no real GPU workload,
# so the decorator is effectively a no-op here; it only exists so ZeroGPU's
# startup check passes and the rest of the (CPU-only) app can run normally.
@spaces.GPU
